...
object Combinator
{
    ...
    def chainr[A, B](p: =>Parsley[A], op: =>Parsley[(A, B) => B], x: B)(implicit @implicitNotFound("Please provide a wrapper function from ${A} to ${B}") wrap: A => B): Parsley[B] = chainr1(p, op).getOrElse(x)
    ...
    /**`chainl(p, op, x)` parses *zero* or more occurrences of `p`, separated by `op`. Returns a value
      ...
      * returned by `p`. If there are no occurrences of `p`, the value `x` is returned.*/
    def chainl[A, B](p: =>Parsley[A], op: =>Parsley[(B, A) => B], x: B)(implicit @implicitNotFound("Please provide a wrapper function from ${A} to ${B}") wrap: A => B): Parsley[B] = chainl1(p, op).getOrElse(x)

    /**`chainr1(p, op)` parses *one* or more occurrences of `p`, separated by `op`. Returns a value
      * obtained by a right associative application of all functions return by `op` to the values
      * returned by `p`.*/
    def chainr1[A, B](p: =>Parsley[A], op: =>Parsley[(A, B) => B])(implicit @implicitNotFound("Please provide a wrapper function from ${A} to ${B}") wrap: A => B): Parsley[B] = new DeepEmbedding.Chainr(p, op, wrap)
    ...
    def chainl1[A, B](p: =>Parsley[A], op: =>Parsley[(B, A) => B])(implicit @implicitNotFound("Please provide a wrapper function from ${A} to ${B}") wrap: A => B): Parsley[B] = new DeepEmbedding.Chainl(+p, op, wrap)

...
private [deepembedding] final class Choice[A](private [backend] val alt1: StrictParsley[A],
    ...
    override def optimise: StrictParsley[A] = this match {
        ...
        case Choice(ret@Choice(_, _, alts: SinglyLinkedList[StrictParsley[A]]), p, alts_) =>
            ...
            alts.stealAll(alts_)
            ret
        case Choice(_, Choice(alt11, alt21, alts1: SinglyLinkedList[StrictParsley[A]]), alts) =>
            assume(!alts1.exists(_.isInstanceOf[Choice[_]]), "alts can never contain a choice")
            assume(!alts.exists(_.isInstanceOf[Choice[_]]), "alts_ can never contain a choice")
            this.alt2 = alt11
            this.alts = alts1
            alts1.prependOne(alt21)
            alts1.stealAll(alts)
            this
        case _ => this
    }

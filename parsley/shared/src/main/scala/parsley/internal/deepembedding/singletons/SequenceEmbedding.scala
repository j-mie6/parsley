...
private [parsley] final class Pure[A](private val x: A) extends Singleton[A] {
    ...
    // $COVERAGE-ON$
    override def genInstrs(producesResults: Boolean)(implicit instrs: InstrBuffer): Unit = instrs += new instructions.Push(x)
...
private [parsley] final class Fresh[A](x: =>A) extends Singleton[A] {
    ...
    // $COVERAGE-ON$
    override def genInstrs(producesResults: Boolean)(implicit instrs: InstrBuffer): Unit = instrs += new instructions.Fresh(x)

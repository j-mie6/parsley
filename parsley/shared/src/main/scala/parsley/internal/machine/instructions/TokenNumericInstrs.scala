...
private [internal] final class TokenSign(ty: SignType, plusPresence: PlusSignPresence) extends Instr {
    ...
    override def apply(ctx: Context): Unit = {
        ...
        if (ctx.moreInput && ctx.peekChar == '-') {
            ...
            ctx.fail(new ClassicExpectedError(ctx.offset, ctx.line, ctx.col, expecteds, 1))
        }
        else {
            ctx.pushError(new ClassicExpectedError(ctx.offset, ctx.line, ctx.col, expecteds, 1))

...
package object debug {
    ...
    implicit class DebugCombinators[P, A](p: P)(implicit con: P => Parsley[A]) {
        /** $debug
          ...
          */
        def debug(name: String, break: Breakpoint, colored: Boolean, watchedRefs: (Ref[?], String)*): Parsley[A] = {
        ...
        private [parsley] def debug(name: String, break: Breakpoint, colored: Boolean): Parsley[A] = {
            debug(name, break, colored, Seq.empty[(Ref[?], String)]*)
        }
        ...
        /** $debug
          ...
          * @param watchedRefs Which references to also track the values of and their names, if any
          */
        def debug(name: String, break: Breakpoint, watchedRefs: (Ref[?], String)*): Parsley[A] =
            debug(name, break, colored = true, watchedRefs*)

        private [parsley] def debug(name: String, break: Breakpoint): Parsley[A] =
            debug(name, break, Seq.empty[(Ref[?], String)]*)
        ...
        /** $debug
          ...
          * @param watchedRefs Which references to also track the values of and their names, if any
          */
        def debug(name: String, colored: Boolean, watchedRefs: (Ref[?], String)*): Parsley[A] =
            debug(name, break = NoBreak, colored, watchedRefs*)

        private [parsley] def debug(name: String, colored: Boolean): Parsley[A] =
            debug(name, colored, Seq.empty[(Ref[?], String)]*)
        ...
        /** $debug
          ...
          * @param watchedRefs Which references to also track the values of and their names, if any
          */
        def debug(name: String, watchedRefs: (Ref[?], String)*): Parsley[A] =
            debug(name, break = NoBreak, colored = true, watchedRefs*)

        private [parsley] def debug(name: String): Parsley[A] = debug(name, Seq.empty[(Ref[?], String)]*)
        ...
        /** Display information about the error messages generated by this parser.
          ...
          * @since 4.0.0
          */
        def debugError(name: String, colored: Boolean)(implicit errBuilder: ErrorBuilder[?]): Parsley[A] = {
        ...
        /** Display information about the error messages generated by this parser.
          ...
          * @since 4.0.0
          */
        def debugError(name: String)(implicit errBuilder: ErrorBuilder[?]): Parsley[A] = debugError(name, colored = true)

...
private [parsley] trait combinator {
    ...
    /** This combinator repeatedly parses a given parser '''zero''' or more times, until the `end` parser succeeds, collecting the results into a `C`.
      ...
      * @since 5.0.0
      */
    final def manyTill[A, C](p: Parsley[A], end: Parsley[_], factory: Factory[A, C]): Parsley[C] = {
    ...
    /** This combinator repeatedly parses a given parser '''one''' or more times, until the `end` parser succeeds, collecting the results into a list.
      ...
      * @since 4.5.0
      */
    final def someTill[A](p: Parsley[A], end: Parsley[_]): Parsley[List[A]] = { // TODO: factory version
        // FIXME: similar to some, I think a someTill(init, p, end) could generalise both manyTill and someTill
        (notFollowedBy(end).ut() ~> (p <::> manyTill(p, end).ut()).ut()).uo("someTill")
    }
    ...
    final def count(min: Int, max: Int)(p: Parsley[_]): Parsley[Int] = min.makeRef { i =>
        exactly(min, p) ~>
        many(guardS(i.gets(_ < max)) ~> p ~> i.update(_ + 1)) ~>
        i.get
    } //TODO: name
}

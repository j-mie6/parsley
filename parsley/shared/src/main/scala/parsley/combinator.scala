...
object combinator {
    ...
    /** This combinator will parse each of `ps` in order, collecting the results.
      ...
      * @note $strict
      */
    def sequence[A](ps: Parsley[A]*): Parsley[List[A]] = sequence[A, List[A]](List, ps: _*)
    private [parsley] def sequence[A, C](factory: Factory[A, C], ps: Parsley[A]*): Parsley[C] = {
        ...
        go(fresh(factory.newBuilder), ps.toList)
    }

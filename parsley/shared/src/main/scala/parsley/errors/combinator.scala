...
object combinator {
    ...
    implicit final class ErrorMethods[P, +A](p: P)(implicit con: P => Parsley[A]) {
        ...
        def filterOut(pred: PartialFunction[A, String]): Parsley[A] = {
            filterWith(con(p))(!pred.isDefinedAt(_), new Parsley(new singletons.VanillaGen(_ => UnexpectedItem.Empty, pred.lift)))
        }
        ...
        def guardAgainst(pred: PartialFunction[A, Seq[String]]): Parsley[A] = {
            filterWith(con(p))(!pred.isDefinedAt(_), new Parsley(new singletons.SpecialisedGen(pred)))
        }
        ...
        def collectMsg[B](msggen: A => Seq[String])(pf: PartialFunction[A, B]): Parsley[B] = {
            collectWith(con(p))(pf, new Parsley(new singletons.SpecialisedGen(msggen)))
        }
        ...
        def unexpectedWhen(pred: PartialFunction[A, String]): Parsley[A] = {
            filterWith(con(p))(!pred.isDefinedAt(_), new Parsley(new singletons.VanillaGen(x => UnexpectedItem.Named(pred(x)), _ => None)))
        }
        ...
        def unexpectedWithReasonWhen(pred: PartialFunction[A, (String, String)]): Parsley[A] = {
            filterWith(con(p))(!pred.isDefinedAt(_), new Parsley(new singletons.VanillaGen(x => UnexpectedItem.Named(pred(x)._1), x => Some(pred(x)._2))))
        }
        ...
        /** This combinator changes the expected component of any errors generated by this parser.
          ...
          * @group rich
          */
        def label(item: String): Parsley[A] = if (item.isEmpty) this.hide else this.labels(item)
        // TODO: document
        def label(item1: String, item2: String, items: String*): Parsley[A] = this.labels((item1 +: item2 +: items): _*)
    ...
    private [parsley] def mapFilterWith[A, B](p: Parsley[A])(f: A => Option[B], err: Parsley[((A, Int)) => Nothing]): Parsley[B] = amendThenDislodge(1) {
        select((offset, entrench(p), offset).zipped { (s, x, e) =>

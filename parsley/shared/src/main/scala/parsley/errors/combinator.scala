...
object combinator {
    ...
    implicit final class ErrorMethods[P, +A](p: P)(implicit con: P => Parsley[A]) {
        ...
        def unexpectedWhen(pred: PartialFunction[A, String]): Parsley[A] = {
            this.filterWith(!pred.isDefinedAt(_), new Parsley(new singletons.VanillaGen(x => UnexpectedItem.Named(pred(x)), _ => None)))
        }
        ...
        def unexpectedWithReasonWhen(pred: PartialFunction[A, (String, String)]): Parsley[A] = {
            this.filterWith(!pred.isDefinedAt(_), new Parsley(new singletons.VanillaGen(x => UnexpectedItem.Named(pred(x)._1), x => Some(pred(x)._2))))
        }

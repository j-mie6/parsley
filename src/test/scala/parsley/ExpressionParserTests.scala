...
class ExpressionParserTests extends ParsleyTest {
    "chain.postfix" must "require an initial value" in {
        chain.postfix('1' #> 1, '+' #> ((x: Int) => x + 1)).runParser("1") should be (Success(1))
    }
    it must "parse all operators that follow" in {
        chain.postfix('1' #> 1, '+' #> ((x: Int) => x + 1)).runParser("1++++++++++++++") should not be a [Failure]
    }
    it must "apply the functions" in {
        chain.postfix('1' #> 1, '+' #> ((x: Int) => x + 1)).runParser("1++++++++++++++") should be (Success(15))
    }
    it must "fail if an operator fails after consuming input" in {
        chain.postfix('1' #> 1, "++" #> ((x: Int) => x + 1)).runParser("1+++++++++++++") shouldBe a [Failure]
    }
    ...
    "chain.prefix" must "parse an operatorless value" in {
        chain.prefix('+' #> ((x: Int) => x + 1), '1' #> 1).runParser("1") should be (Success(1))
    }
    it must "parse all operators that precede a value" in {
        chain.prefix('+' #> ((x: Int) => x + 1), '1' #> 1).runParser("+++++++++++1") should not be a [Failure]
    }
    it must "fail if the final value is absent" in {
        chain.prefix('+' #> ((x: Int) => x + 1), '1' #> 1).runParser("+++++++++++") shouldBe a [Failure]
    }
    it must "apply the functions" in {
        chain.prefix('+' #> ((x: Int) => x + 1), '1' #> 1).runParser("+++++++++++1") should be (Success(12))
    }

    "chain.right1" must "require an initial value" in {
        chain.right1("11" #> 1, '+' #> ((x: Int, y: Int) => x + y)).runParser("11") should be (Success(1))
        chain.right1("11" #> 1, '+' #> ((x: Int, y: Int) => x + y)).runParser("1") shouldBe a [Failure]
        chain.right1("11" #> 1, '+' #> ((x: Int, y: Int) => x + y)).runParser("2") shouldBe a [Failure]
    }
    it must "parse all operators and values that follow" in {
        chain.right1("11" #> 1, '+' #> ((x: Int, y: Int) => x + y)).runParser("11+11+11+11+11") should be (Success(5))
    }
    it must "apply the functions with correct associativity" in {
        chain.right1(digit.map(_.asDigit), '%' #> ((x: Int, y: Int) => x % y)).runParser("6%5%2%7") should be (Success(0))
    }
    it must "fail if an operator or p fails after consuming input" in {
        chain.right1("11" #> 1, "++" #> ((x: Int, y: Int) => x + y)).runParser("11+11+11+11+11") shouldBe a [Failure]
        chain.right1("11" #> 1, "++" #> ((x: Int, y: Int) => x + y)).runParser("11++11++11++1++11") shouldBe a [Failure]
    }
    ...
    "chain.right" must "allow for no initial value" in {
        chain.right("11" #> 1, '+' #> ((x: Int, y: Int) => x + y), 0).runParser("2") shouldBe Success(0)
        chain.right("11" #> 1, '+' #> ((x: Int, y: Int) => x + y), 0).runParser("1") shouldBe a [Failure]
    }

    "chain.left1" must "require an initial value" in {
        chain.left1("11" #> 1, '+' #> ((x: Int, y: Int) => x + y)).runParser("11") should be (Success(1))
        chain.left1("11" #> 1, '+' #> ((x: Int, y: Int) => x + y)).runParser("1") shouldBe a [Failure]
        chain.left1("11" #> 1, '+' #> ((x: Int, y: Int) => x + y)).runParser("2") shouldBe a [Failure]
    }
    it must "parse all operators and values that follow" in {
        chain.left1("11" #> 1, '+' #> ((x: Int, y: Int) => x + y)).runParser("11+11+11+11+11") should be (Success(5))
    }
    it must "apply the functions with correct associativity" in {
        chain.left1(digit.map(_.asDigit), '%' #> ((x: Int, y: Int) => x % y)).runParser("6%5%2%7") should be (Success(1))
    }
    it must "fail if an operator fails after consuming input" in {
        chain.left1("11" #> 1, "++" #> ((x: Int, y: Int) => x + y)).runParser("11+11+11+11+11") shouldBe a [Failure]
        chain.left1("11" #> 1, "++" #> ((x: Int, y: Int) => x + y)).runParser("11++11++11++1++11") shouldBe a [Failure]
    }
    it must "not leave the stack in an inconsistent state on failure" in {
    ...
    it must "correctly accept the use of a wrapping function" in {
        ...
        case class Num(x: Int) extends Expr
        chain.left1[Int, Expr]("1" #> 1, "+" #> Add.apply)(Num).runParser("1+1+1") should be (Success(Add(Add(Num(1), 1), 1)))
    }
    "chain.left" must "allow for no initial value" in {
        chain.left("11" #> 1, '+' #> ((x: Int, y: Int) => x + y), 0).runParser("11") should be (Success(1))
        chain.left("11" #> 1, '+' #> ((x: Int, y: Int) => x + y), 0).runParser("1") shouldBe a [Failure]
        chain.left("11" #> 1, '+' #> ((x: Int, y: Int) => x + y), 0).runParser("2") shouldBe Success(0)
    }

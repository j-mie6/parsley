...
class Lexer(lang: LanguageDef)
{
    private def keyOrOp(startImpl: Impl, letterImpl: Impl, parser: Parsley[String], illegal: String => Boolean,
        ...
        val builder = (start: Char => Boolean, letter: Char => Boolean) =>
            new Parsley(new frontend.NonSpecific(combinatorName, name, illegalName, start, letter, illegal))
        lexeme {
    ...
    {
        case Static(letter) => lexeme(new Parsley(new frontend.Specific("keyword", name, letter, lang.caseSensitive)))
    ...
    {
        case Static(letter) => new Parsley(new frontend.Specific("operator", name, letter, true))
    ...
    /**The non-lexeme parser `maxOp_(name)` parses the symbol `name`, but also checks that the `name`
      * is not part of a larger reserved operator. An `operator` is treated as a single token using
      * `attempt`.*/
    def maxOp_(name: String): Parsley[Unit] = void(new Parsley(new frontend.MaxOp(name, lang.operators)))
    ...
    {
        case Static(ws) => new Parsley(new frontend.StringLiteral(ws))
    ...
    lazy val rawStringLiteral: Parsley[String] = new Parsley(frontend.RawStringLiteral)

    private def letter(terminal: Char): Parsley[Char] = satisfy(c => c != terminal && c != '\\' && c > '\u0016')

    private lazy val escapeCode = new Parsley(frontend.Escape)
    ...
    // Floats
    private def sign(ty: SignType) = new Parsley(new frontend.Sign[ty.resultType](ty))
    private lazy val floating = new Parsley(frontend.Float)
    ...
    private lazy val nat = new Parsley(frontend.Natural)
    ...
    val whiteSpace_ : Impl => Parsley[Unit] =
    {
        ...
        case Static(ws) => new Parsley(new frontend.WhiteSpace(ws, lang.commentStart, lang.commentEnd, lang.commentLine, lang.nestedComments))
        case Parser(space_) if lang.supportsComments =>
            skipMany(attempt(new Parsley(new frontend.Comment(lang.commentStart, lang.commentEnd, lang.commentLine, lang.nestedComments))) <|> space_)
    ...
    lazy val skipComments: Parsley[Unit] = {
        if (!lang.supportsComments) unit
            ...
            new Parsley(new frontend.SkipComments(lang.commentStart, lang.commentEnd, lang.commentLine, lang.nestedComments))
        }
    }

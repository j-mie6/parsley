...
private [deepembedding] sealed abstract class BranchLike[A, B, C, D](instr: Int => instructions.Instr, finaliser: Option[instructions.Instr])
...
private [parsley] final class If[A](val b: StrictParsley[Boolean], val p: StrictParsley[A], val q: StrictParsley[A])
...
private [parsley] final class FastFail[A](var p: StrictParsley[A], msggen: A => String)
    extends FastZero[A](x => new Fail(msggen(x)), new instructions.FastFail(msggen)) with MZero
private [parsley] final class FastUnexpected[A](var p: StrictParsley[A], msggen: A => String)
    extends FastZero[A](x => new Unexpected(msggen(x)), new instructions.FastUnexpected(msggen)) with MZero

private [deepembedding] sealed abstract class FilterLike[A](fail: A => StrictParsley[Nothing], instr: instructions.Instr, pred: A => Boolean)
    extends Unary[A, A] {
...
private [parsley] final class Filter[A](var p: StrictParsley[A], pred: A => Boolean)
    extends FilterLike[A](_ => Empty, new instructions.Filter(pred), !pred(_))
private [parsley] final class FilterOut[A](var p: StrictParsley[A], pred: PartialFunction[A, String])
    extends FilterLike[A](x => ErrorExplain(Empty, pred(x)), new instructions.FilterOut(pred), pred.isDefinedAt(_))
private [parsley] final class GuardAgainst[A](var p: StrictParsley[A], pred: PartialFunction[A, String])

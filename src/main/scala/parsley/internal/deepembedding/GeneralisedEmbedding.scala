...
private [parsley] abstract class Singleton[A](pretty: String, strict: StrictParsley[A]) extends Parsley[A] {
    ...
    final override def preprocess[Cont[_, +_], R, A_ >: A](implicit ops: ContOps[Cont, R],
...
private [deepembedding] abstract class Unary[A, B](p: Parsley[A], pretty: String => String, make: StrictParsley[A] => StrictParsley[B])
    ...
    override def preprocess[Cont[_, +_], R, B_ >: B](implicit ops: ContOps[Cont, R],
...
private [deepembedding] abstract class Binary[A, B, C](left: Parsley[A], _right: =>Parsley[B],
                                                       pretty: (String, String) => String, make: (StrictParsley[A], StrictParsley[B]) => StrictParsley[C]) extends Parsley[C] {
    ...
    final override def preprocess[Cont[_, +_], R, C_ >: C](implicit ops: ContOps[Cont, R],
...
private [deepembedding] abstract class Ternary[A, B, C, D](first: Parsley[A], _second: =>Parsley[B], _third: =>Parsley[C],
                                                           pretty: (String, String, String) => String, make: (StrictParsley[A], StrictParsley[B], StrictParsley[C]) => StrictParsley[D])
    extends Parsley[D] {
    private lazy val second: Parsley[B] = _second
    private lazy val third: Parsley[C] = _third
    final override def findLetsAux[Cont[_, +_], R](seen: Set[Parsley[_]])
        (implicit ops: ContOps[Cont, R], state: LetFinderState): Cont[R, Unit] = {
        first.findLets(seen) >> second.findLets(seen) >> third.findLets(seen)
    }
    final override def preprocess[Cont[_, +_], R, D_ >: D](implicit ops: ContOps[Cont, R],

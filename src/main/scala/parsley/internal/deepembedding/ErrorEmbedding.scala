...
private [parsley] final class ErrorLabel[A](p: Parsley[A], private [ErrorLabel] val label: String)
    extends ScopedUnary[A, A](p, s"label($label)", new backend.ErrorLabel(_, label))
private [parsley] final class ErrorExplain[A](p: Parsley[A], reason: String)

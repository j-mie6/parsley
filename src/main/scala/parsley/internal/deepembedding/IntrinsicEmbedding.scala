...
private [parsley] final class Modify[S](r: Reg[S], f: S => S) extends Singleton[Unit](s"modify($r, ?)", new instructions.Modify(r.addr, f))
private [parsley] final class Local[S, A](r: Reg[S], _p: =>Parsley[S], _q: =>Parsley[A])
    ...
    override def codeGen[Cont[_, +_]: ContOps](implicit instrs: InstrBuffer, state: CodeGenState): Cont[Unit, Unit] = {
        left.codeGen >> {
            instrs += new instructions.LocalEntry(r.addr)
            right.codeGen |>
            (instrs += new instructions.LocalExit(r.addr))
        }
    }
}

...
private [parsley] abstract class Parsley[+A] private [deepembedding] {
    ...
    // Internals
    final private [deepembedding] def findLets[Cont[_, +_], R](seen: Set[Parsley[_]])(implicit ops: ContOps[Cont], state: LetFinderState): Cont[R, Unit] = {
        state.addPred(this)
    ...
    final private [deepembedding] def optimised[Cont[_, +_], R, A_ >: A](implicit ops: ContOps[Cont],
    ...
    final private [deepembedding] def unsafeOptimised[Cont[_, +_], R, A_ >: A](implicit ops: ContOps[Cont],
                                                                                        lets: LetMap, recs: RecMap): Cont[R, StrictParsley[A_]] = {
    ...
    private [deepembedding] def preprocess[Cont[_, +_], R, A_ >: A](implicit ops: ContOps[Cont], lets: LetMap, recs: RecMap): Cont[R, StrictParsley[A_]]
    // Let-finder recursion
    protected def findLetsAux[Cont[_, +_], R](seen: Set[Parsley[_]])(implicit ops: ContOps[Cont], state: LetFinderState): Cont[R, Unit]
...
private [deepembedding] object LetMap {
    def apply[Cont[_, +_]](lets: Iterable[Parsley[_]])(implicit ops: ContOps[Cont], recs: RecMap): LetMap = {

...
private [parsley] final class <*>[A, B](_pf: StrictParsley[A => B], _px: =>Parsley[A])
    extends Binary[A => B, A, B](_pf.asInstanceOf, _px, (l, r) => s"($l <*> $r)", new <*>(_, ???)) {
...
private [parsley] final class >>=[A, B](_p: StrictParsley[A], private [>>=] val f: A => Parsley[B])
    extends Unary[A, B](_p.asInstanceOf, l => s"($l >>= ?)", new >>=(_, f)) {
    override val numInstrs = 1
...
private [deepembedding] sealed abstract class Seq[A, B, Res](_left: StrictParsley[A], _right: =>Parsley[B], pretty: String, make: StrictParsley[A]=>Seq[A, B, Res])
    extends Binary[A, B, Res](_left.asInstanceOf, _right, (l, r) => s"($l $pretty $r)", make) {
    ...
    protected def makeSeq(str: Parsley[String], res: Pure[Res]): StrictParsley[Res]
...
private [parsley] final class *>[A](_p: StrictParsley[_], _q: =>Parsley[A]) extends Seq[Any, A, A](_p, _q, "*>", new *>(_, ???)) {
    ...
    def makeSeq(str: Parsley[String], res: Pure[A]): StrictParsley[A] = {
        discard = str
        result = res
        optimise
    }
    ...
    @tailrec override def optimise: StrictParsley[A] = optimiseSeq match {
        ...
        case None => discard match {
            ...
            case u => result match {
                ...
                case v *> w =>
                    discard = *>(u, v).optimiseDefinitelyNotTailRec.asInstanceOf
                    result = w
                    optimise
                case _ => this
            }
...
private [parsley] final class <*[A](_p: StrictParsley[A], _q: =>Parsley[_]) extends Seq[A, Any, A](_p, _q, "<*", new <*(_, ???)) {
    ...
    @tailrec override def optimise: StrictParsley[A] =  optimiseSeq match {
        ...
        case None => discard match {
            ...
            case w => result match {
                ...
                case u: Pure[_] => *>(w.asInstanceOf, u).optimise
                ...
                case u <* v =>
                    result = u
                    discard = <*(v, w.asInstanceOf).optimiseDefinitelyNotTailRec.asInstanceOf
                    optimise
                case _ => this
            }
        }
...
private [deepembedding] object <*> {
    ...
    def unapply[A, B](self: <*>[A, B]): Some[(Parsley[A=>B], Parsley[A])] = Some((self.left, self.right.asInstanceOf))
}
private [deepembedding] object >>= {
...
private [deepembedding] object Seq {
    def unapply[A, B, Res](self: Seq[A, B, Res]): Some[(Parsley[_], Parsley[Res])] = Some((self.discard.asInstanceOf, self.result.asInstanceOf))
}
private [deepembedding] object *> {
    def apply[A](left: Parsley[_], right: Parsley[A]): *>[A]= new *>(left, ???).ready(right)
    def unapply[A](self: *>[A]): Some[(Parsley[_], Parsley[A])] = Some((self.discard, self.result.asInstanceOf))
}
private [deepembedding] object <* {
    def apply[A](left: Parsley[A], right: Parsley[_]): <*[A] = new <*(left, ???).ready(right)
    def unapply[A](self: <*[A]): Some[(Parsley[A], Parsley[_])] = Some((self.result, self.discard.asInstanceOf))
}
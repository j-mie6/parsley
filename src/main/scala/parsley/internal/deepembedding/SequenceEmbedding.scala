...
private [deepembedding] sealed abstract class Seq[A, B, Res](_left: Parsley[A], _right: =>Parsley[B], pretty: String, make: Parsley[A]=>Seq[A, B, Res])
    ...
    final protected def optimiseSeq: Option[Parsley[Res]] = discard match {
        ...
        case u *> (_: Pure[_]) =>
            discard = u
            Some(this.optimise.asInstanceOf[Parsley[Res]])
    ...
    @tailrec override def optimise: Parsley[A] = optimiseSeq match {
        ...
        case None => discard match {
            ...
            case u => result match {
                ...
                case v *> w =>
                    discard = *>(u, v).optimiseDefinitelyNotTailRec.asInstanceOf[Parsley[A]]
                    result = w
                    optimise
                case _ => this
            }
    ...
    @tailrec override def optimise: Parsley[A] =  optimiseSeq match {
        ...
        case None => discard match {
            ...
            case w => result match {
                ...
                case u <* v =>
                    result = u
                    discard = <*(v, w).optimiseDefinitelyNotTailRec.asInstanceOf[Parsley[_]]
                    optimise
                case _ => this
            }
        }

...
private [parsley] final class ChainPre[A](p: LazyParsley[A], op: =>LazyParsley[A => A]) extends Binary[A, A => A, A](p, op) {
    override def pretty(l: String, r: String) = s"chainPre($r, $l)"
    override def make(p: StrictParsley[A], op: StrictParsley[A => A]) = new backend.ChainPre(p, op)
}

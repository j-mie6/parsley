...
/** This module contains a huge number of pre-made combinators that are very useful for a variety of purposes.
  ...
  * @groupdesc alt
  *     These combinators allow for testing a large number of parsers in one go. Be careful, however, these are
  ...
  *     is a neat trick: instead of writing `choice(p, q, .., r)`, write `p <|> choice(q, .., r)`, which
  *     will leverage the laziness of `<|>` to ensure the entire `choice` combinator is left lazily evaluated.
  *     For `attemptChoice`, the same applies but with `attempt(p) <|> ..`.
  *
  * @groupprio cond 75
